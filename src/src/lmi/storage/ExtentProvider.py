...
class ExtentProvider(DeviceProvider):
    ...
    def provides_name(self, object_name):
        ...
        if not (object_name.has_key('SystemName')
            ...
            return False
        if object_name['SystemName'] != get_system_name():
            return False
    ...
    @cmpi_logging.trace_method
    def get_instance_id(self, device):
        """
        Return InstanceID for given device.

        :param device: (``StorageDevice``) The device.
        :returns: ``string``.
        """
        return ("LMI:" + self.classname + ":"
                + storage.get_persistent_name(device))
    ...
    def get_instance(self, env, model, device=None):
        ...
        model['InstanceID'] = self.get_instance_id(device)
